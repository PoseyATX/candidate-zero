 ...
 * PR01: full-bleed promo art inlined so it cannot 404.
 */
...
/** Registered rasters under public/assets/cards/ (optional; PR01 is inlined). */
export const CARD_ART: Record<string, CardArtEntry> = {
  PR01: { file: 'PR01.svg', fullFace: true }
};

/** Full-bleed faces: art is the entire 2:3 card (title + cost in the art). */
const FULL_FACE_ART = new Set(['PR01']);

/**
 * PR01 full face — inlined so Pages/cache/path can never fall back to the star shell.
 * Matches the supporter promo design: pink wash, gold star, title, FREE seal.
 */
...
export function artPlateHtml(cardId: string): string {
  // Inline full-face for PR01 — never depends on a deployed asset path.
  if (cardId === 'PR01') {
...
export function cardInner(
  ...
  // Full-face promo: art is the entire 2:3 face (title + cost painted in art).
